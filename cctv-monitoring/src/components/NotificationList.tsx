...
import { extractCameraInfo, formatTimestampFromPath } from '../utils/filePathUtils';
...
const NotificationList: React.FC = () => {
  ...
  const [selectedNotification, setSelectedNotification] = useState<ShopliftingNotification | null>(null);
  ...
  const filteredAndSortedNotifications = useMemo(() => {
    ...
    if (filterCamera !== 'all') {
      filtered = filtered.filter(n => extractCameraInfo(n.videoPath) === filterCamera);
    }
  ...
  const handleConfirm = async (isShoplifting: boolean) => {
    ...
    try {
      ...
      if (selectedNotification.video_id) {
        ...
        await apiService.confirmVideo(selectedNotification.videoPath, isShoplifting);
      } else {
    ...
    <>
      <Card>
        <Card.Header className="d-flex justify-content-between align-items-center">
          ...
          <div className="text-muted small">
            {filteredAndSortedNotifications.length} of {notifications.length} {notifications.length === 1 ? 'notification' : 'notifications'}
          </div>
        </Card.Header>
            ...
            <Col md={4}>
              <Form.Group>
                ...
                <div className="d-flex">
                  ...
                  />
                  {(filterStatus !== 'all' || filterCamera !== 'all' || searchQuery) && (
                    <Button 
        ...
        <Card.Body className="p-0 pt-2">
          {filteredAndSortedNotifications.length === 0 ? (
            <p className="text-muted p-3 text-center">
              {notifications.length > 0 ? 'No notifications match your filters' : 'No shoplifting notifications'}
            </p>
          ) : (
            <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
              <ListGroup variant="flush">
                {filteredAndSortedNotifications.map((notification) => (
                  ...
                  >
                    <div>
                      <div><strong>{extractCameraInfo(notification.videoPath)}</strong></div>
                      <div>Video Time: {formatTimestampFromPath(notification.videoPath)}</div>
                      <div>Detected: {formatDate(notification.dateDetected)}</div>
                      <div className="text-muted small">{notification.videoPath}</div>
                    </div>
                    <div className="d-flex align-items-center">
                      {notification.confirmed === null ? (
                        <Badge bg="warning" className="me-2">Unconfirmed</Badge>
                      ) : notification.confirmed ? (
                        <Badge bg="danger" className="me-2">Confirmed Shoplifting</Badge>
                      ) : (
                        <Badge bg="success" className="me-2">False Alarm</Badge>
                      )}
                      <Button
                        variant="outline-primary"
                        size="sm"
                        onClick={() => handleView(notification)}
                        disabled={notification.confirmed !== null}
                      >
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>
            Review Potential Shoplifting Incident - {selectedNotification && extractCameraInfo(selectedNotification.videoPath)}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedNotification && (
            <>
              ...
              <div className="ratio ratio-16x9 mb-3">
                <video controls>
                  <source src={apiService.getVideoUrl(selectedNotification.videoPath)} type="video/mp4" />
                  Your browser does not support the video tag.
                </video>
              </div>
              
              <p><strong>Camera:</strong> {extractCameraInfo(selectedNotification.videoPath)}</p>
              <p><strong>Video Time:</strong> {formatTimestampFromPath(selectedNotification.videoPath)}</p>
              <p><strong>Detection Time:</strong> {formatDate(selectedNotification.dateDetected)}</p>
              <p><strong>File:</strong> {selectedNotification.videoPath}</p>
              
              {selectedNotification.confirmed === null ? (
                <p className="text-warning">Please confirm if this is actually shoplifting or a false alarm.</p>
              ) : (
                <p className={selectedNotification.confirmed ? 'text-danger' : 'text-success'}>
                  This was marked as {selectedNotification.confirmed ? 'shoplifting' : 'not shoplifting'}.
                </p>
              )}
            </>
          )}
